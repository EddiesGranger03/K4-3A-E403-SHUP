# Thư mục Thu hoạch Cá nhân (Reflection) — Nhóm SHUP (Phòng E403)

Mục đích: Chứa bài viết thu hoạch phản tư cá nhân (Personal Reflection) của từng thành viên trong nhóm SHUP trước khi kết thúc cuộc thi Hackathon AI Batch 04 · VinUni 2026.

## 👥 Danh sách bài viết phản tư theo phân bổ vai trò:

| STT | Thành viên | Mã học viên | Vai trò đảm nhiệm | Tệp Reflection chi tiết |
|---|---|---|---|---|
| 1 | **Nguyễn Khánh Sơn** | 2A202602388 | **Đội trưởng kiêm Kỹ sư Nòng cốt (Product Lead & Lead Builder)** | [`reflection_son.md`](reflection_son.md) |
| 2 | **Bùi Thị Thu Uyên** | 2A202602613 | **User Research & Data Mining Lead** | [`reflection_uyen.md`](reflection_uyen.md) |
| 3 | **Lê Châu Trần Phát** | 2A2026025245 | **AI & Evaluation Lead** | [`reflection_phat.md`](reflection_phat.md) |
| 4 | **Ngô Xuân Hoàng** | 2A202602597 | **Frontend UI/UX & Testing Engineer** | [`reflection_hoang.md`](reflection_hoang.md) |

### 📌 Nội dung chuẩn hóa trong từng bài viết:
1. **Vai trò & Trách nhiệm:** Phân định rõ ràng mảng việc phụ trách trong 47.5h.
2. **Những quyết định then chốt & Thách thức vượt qua:** Cụ thể hóa bằng sự kiện, con số và mã bằng chứng trong dữ liệu thật.
3. **Vấp ngã & Bài học xương máu:** Khủng hoảng độ trễ token, lỗi kiểm thử Run 1, bẫy suy diễn hộ người dùng...
4. **Sự chuyển biến về tư duy sản phẩm AI:** Chuyển đổi từ tư duy code truyền thống sang tư duy hệ thống AI xác suất, trách nhiệm và có căn cứ.
5. **Kế hoạch phát triển tiếp theo:** Roadmap hoàn thiện và ứng dụng vào thực tế.
